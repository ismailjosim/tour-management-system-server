...
const changePasswordIntoDB = async (
  ...
  if (newPassword.includes(user!.email)) {
    throw new AppError(httpStatus.BAD_REQUEST, 'Password can not includes user email');
  }
...
const forgotPasswordIntoDB = async (email: string) => {
  ...
  const resetToken = jwt.sign(jwtPayload, environmentVariables.JWT_ACCESS_SECRET, {
    expiresIn: '10m',
  });
...
const resetPasswordIntoDB = async (payload: Record<string, any>, decodedToken: JwtPayload) => {
