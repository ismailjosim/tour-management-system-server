...
const getAllDivisions = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const result = await DivisionServices.getAllDivisionFromDB(req.query as Record<string, string>);
  sendResponse(res, {
    success: true,
...
const getSingleDivision = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const result = await DivisionServices.getSingleDivisionFromDB(req.params.slug as string);
  sendResponse(res, {
    success: true,
    statusCode: httpStatus.OK,
    message: result.data ? 'Division Retried successfully' : 'No Division Found',
    data: result.data,
  });
});
...
const updateDivision = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const result = await DivisionServices.updateDivisionIntoDB(id, payload);
  sendResponse(res, {
    success: true,
    statusCode: httpStatus.OK,
...
const deleteDivision = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const result = await DivisionServices.deleteDivisionFromDB(req.params.id as string);
  sendResponse(res, {
    success: true,
    statusCode: httpStatus.OK,
