...
const updateUserIntoDB = async (
  ...
  if (
    ...
    throw new AppError(httpStatus.FORBIDDEN, 'The SUPER_ADMIN role cannot be changed');
  }
  ...
  if (payload.isActive || payload.isDeleted || payload.isVerified) {
    if (decodedToken.role === Role.USER || decodedToken.role === Role.GUIDE) {
      throw new AppError(httpStatus.FORBIDDEN, 'You are not authorized to update system fields');
    }
  }
...
const updateUserProfilePictureIntoDB = async (
  ...
  if (userId !== decodedToken.userId) {
    throw new AppError(httpStatus.FORBIDDEN, 'You can only update your own profile picture');
  }
