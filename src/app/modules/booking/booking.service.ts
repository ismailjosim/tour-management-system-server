...
const assertBookingAccess = (bookingUser: any, decodedUser: JwtPayload) => {
  if (!isAdminRole(decodedUser.role) && getBookingUserId(bookingUser) !== decodedUser.userId) {
...
const createBookingIntoDB = async (payload: Partial<IBooking>, userId: string) => {
  ...
  try {
    ...
    // 3️⃣ Fetch Tour Info
    const tour = await TourModel.findById(payload.tour).select('costFrom').session(session);
    if (!tour?.costFrom) {
...
const getUserBookingFromDB = async (userId: string, query: Record<string, string>) => {
  const queryBuilder = new QueryBuilder(
...
const getSingleBookingFromDB = async (bookingId: string, decodedUser: JwtPayload) => {
